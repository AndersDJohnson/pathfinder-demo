...
(function () {
            ...
            if (name) {
                if (prefix) {
                    ...
                    if (!url) {
                        ...
                        //messy to support.
						var ext = null;
						if ( config.extensions && config.extensions[name] ) ext = config.extensions[name];
                        url = context.nameToUrl(name, ext, parentModuleMap);
    ...
    req.attach = function (url, context, moduleName, callback, type, fetchOnlyFunction) {
        ...
        if (isBrowser) {
            ...
            node.type = type ||
			            //(context && context.config.scriptTypes && context.config.scriptTypes[moduleName]) ||
			            (context && context.config.scriptType) ||
                        "text/javascript";
            node.charset = "utf-8";
